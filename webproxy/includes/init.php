...
$phrases['no_hotlink']      = '盗链是不允许的。';
$phrases['unique_mismatch'] = '这个独特的URL请求和你的缓存不匹配 - 这个URL不是为您生存的或者已过期。';
$phrases['invalid_url']     = '请求的URL不是有效的URL。 曾尝试载入: %s';
$phrases['banned_site']     = '抱歉，这个代理不允许请求的站点 (<b>%s</b>) 被显示。';
$phrases['file_too_large']  = '请求的文件过大。允许的最大文件大小是 %s MB。';
$phrases['server_busy']     = '服务器忙，无法处理你的请求。请稍后再试。我们为您的不便抱歉。';
$phrases['http_error']      = '请求的资源无法载入。因为服务器返回了如下错误:<br> &nbsp; <b>%s %s</b> (<span class="tooltip" onmouseout="exit()" onmouseover="tooltip(\'%s\');">?</span>)。';
$phrases['curl_error']      = '请求的资源无法载入。libcurl返回了如下错误:<br><b>%s</b>';
$phrases['unknown_error']   = '脚本遇到了未知错误。错误id: <b>%s</b>.';
...
$httpErrors = array('404' => '当请求的资源不存在时，404错误就会出现。');
...
function proxifyURL($url, $givenFlag = false) {
   ...
   if ( $options['encodeURL'] ) {
      ...
      // Apply base64
      $url = base64_encode(base64_encode($url));
   ...
   if ( ! isset($phrases[$type]) ) {
	...
	if ( isset($flag) && ( $flag == 'frame' || $flag == 'ajax' ) ) {
      ...
      if ( $args ) {
		...
		die($errorText . ' <a href="index.php">返回首页</a>.');
